...
def configure(
    ...
    """
    Configure SnackTensors library settings and start the gRPC server.
    ...
    logger.info(f"SnackTensors configured: storage_path={storage_path}, mem_pool_size={mem_pool_size//1024**3}GB")
    ...
    if ensure_server_running():
        ...
        raise RuntimeError("Failed to start SnackTensors gRPC server")
...
def connect(
    ...
    """
    Connect to an existing SnackTensors gRPC server without starting a new one.
    ...
    try:
        ...
        logger.info(f"✅ Successfully connected to SnackTensors server at {get_server_address()}")
        
    except Exception as e:
        logger.error(f"❌ Failed to connect to SnackTensors server at {server_host}:{server_port}")
        raise ConnectionError(f"No SnackTensors server running at {server_host}:{server_port}. Please start the server first with 'flash start'") from e
...
def shutdown_server() -> None:
    """
    Shutdown the SnackTensors gRPC server.
    
    This stops the storage server and supervisord daemon.
    After calling this, you can reconfigure SnackTensors settings
    ...
    try:
        ...
        logger.info("🛑 Shutting down SnackTensors server...")
        server_manager.stop_server()
        logger.info("✅ SnackTensors server shutdown complete")
    except Exception as e:
